...
import { submitTaskAttempt } from "@/features/exam-task-runtime/attempt-service";
import { examTaskRuntimeErrorResponse } from "@/features/exam-task-runtime/route-utils";
import { prisma } from "@/lib/db/client";

export function createSubmitTaskAttemptRoute(
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  return createSubmitTaskAttemptRoute({ db: prisma }, id)(request);
}
