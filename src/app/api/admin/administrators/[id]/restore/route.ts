...
import { restoreAdminAccount } from "@/features/admin-accounts/admin-account-service";
import {
...
export function createAdministratorRestoreRoute(
  { db }: { db: PrismaClient },
  ...
  return async function POST(request: Request) {
    try {
      ...
      const admin = await restoreAdminAccount(db, administratorId, actor.id);
...
export async function POST(
  ...
  return createAdministratorRestoreRoute({ db: prisma }, id)(request);
}
